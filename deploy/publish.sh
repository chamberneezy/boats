...
for lake in ${LAKES}; do
  echo "== Building ${lake} =="
  npm run build:data -- --lake "${lake}" --out "${OUT}"
done

echo "== Publishing to ${DATA_BUCKET} =="
...
gcloud storage rsync --recursive "${OUT}" "${DATA_BUCKET}"

# Cache policy: the tiny manifest is re-checked often; the hashed timetable never changes in place.
# Best-effort (|| true) so a transient metadata hiccup does not fail an otherwise-good publish.
gcloud storage objects update "${DATA_BUCKET}/**/timetable.*.json" \
  --cache-control="public, max-age=31536000, immutable" || true
gcloud storage objects update "${DATA_BUCKET}/**/manifest.json" \

...
// Placeholder from the design ("Map placeholder — <lake> piers to be mapped here").
// The map itself isn't built yet, so its action is shown disabled.
export function MapPlaceholder({ lakeName, className = '' }: { lakeName: string; className?: string }) {
  return (
    <div
    ...
    >
      <div className="flex min-h-[160px] flex-1 flex-col items-center justify-center gap-1 lg:min-h-[280px]">
        ...
        <span className="font-body text-[11px] text-stone-grey lg:text-xs">{lakeName} piers to be mapped here</span>
      </div>

...
export function SearchLayout({ children, lakeName, showMapOnPhone }: SearchLayoutProps) {
  return (
    <main className="mx-auto max-w-[1440px] px-5 pb-16 pt-2 md:px-16 md:pt-14">
      ...
      <div className="flex flex-col gap-8 md:mt-8 lg:flex-row lg:items-start">
        ...
        <MapPlaceholder
          lakeName={lakeName}
          className={`min-w-[320px] flex-1 lg:flex lg:self-stretch ${showMapOnPhone ? 'flex' : 'hidden'}`}
        />
      </div>
    </main>
  );
